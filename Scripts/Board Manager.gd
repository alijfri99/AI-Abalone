...
func _ready():
	...
	var l = find_two_clusters(current_state, BLACK)
	print(l)
...
func find_two_clusters(state, element): # finds all the 2-element clusters of 'element' in a state
	...
	for row in range(len(state)):
		for column in range(len(state[row])):
			#if check_two_s_cluster(state, row, column, element) == true:
				#var new_two_s_cluster = Cluster.new(row, column, Cluster.TWO_S)
				#result.append(new_two_s_cluster)
				
			#if check_two_ld_cluster(state, row, column, element) == true:
				#var new_two_ld_cluster = Cluster.new(row, column, Cluster.TWO_LD)
				#result.append(new_two_ld_cluster)
				
			if check_two_rd_cluster(state, row, column, element) == true:
				var new_two_rd_cluster = Cluster.new(row, column, Cluster.TWO_RD)
				result.append(new_two_rd_cluster)
	return result
			
func check_two_s_cluster(state, row, column, element): # checks the existence of a 'TOW_S' cluster of type 'element' in a state, starting from a specific row and column
...
func check_two_ld_cluster(state, row, column, element): # checks the existence of a 'TOW_LD' cluster of type 'element' in a state, starting from a specific row and column
...
func check_two_rd_cluster(state, row, column, element): # checks the existence of a 'TOW_RD' cluster of type 'element' in a state, starting from a specific row and column
