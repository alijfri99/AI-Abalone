...
func check_one_cluster(board, cell_number, piece): # checks if there is a one-element cluster of type 'piece' at a given cell number on a board
...
func check_three_cluster(board, cell_number, direction, piece): # checks if there is a three-element cluster of type 'piece' in a specific direction at a given cell number on a board
	var neighbor_number = BoardManager.neighbors[cell_number][direction]
